...
public:
    ...
    Sphere(unsigned int dMap, unsigned int sMap, float textureXmin, float textureYmin, float textureXmax, float textureYmax, float radius = 0.5f, int sectorCount = 48, int stackCount = 18, glm::vec3 amb = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 diff = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f) : verticesStride(24)
    {
        ...
        buildHalfSphereCoordinatesAndIndices();
        buildVertices();
    ...
    void buildHalfSphereCoordinatesAndIndices(bool upperHemisphere = true)
    {
        ...
        int startStack = upperHemisphere ? 0 : stackCount / 2;
        int endStack = upperHemisphere ? stackCount / 2 : stackCount;

        for (int i = startStack; i <= endStack; ++i)
        {
            stackAngle = PI / 2 - i * stackStep;        // starting from pi/2 to 0 or 0 to -pi/2
        ...
        for (int i = startStack; i < endStack; ++i)
        {
            ...
            {
                if (upperHemisphere || i != stackCount - 1)  // Avoid duplicates
                {
